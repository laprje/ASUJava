...
// FILENAME: Lab5.java
// SPECIFICATION: advanced loops; making patterns with characters
// FOR: CSE 110
// TIME SPENT:
//-----------------------------------------------------------*/
...
public class Lab5 {
    ...
    private static String buildDiamond(
        ...
        if(numSymbols%2 == 0){
            symbMiddle = numSymbols -1;
        }
        else {
            symbMiddle = numSymbols;
        }
            for(int numStars = 1; numStars <= symbMiddle; numStars+=2) {
                int numSpaced = (symbMiddle - numStars)/2;
                String row = buildRow(sideSymb, numSpaced, midSymb, numStars);
                result += row + "\n";
            }
            for(int numStars = symbMiddle-2; numStars > 0; numStars-=2) {
                int numSpaced = (symbMiddle - numStars)/2;
                String row = buildRow(sideSymb, numSpaced, midSymb, numStars);
                result += row + "\n";
        }
